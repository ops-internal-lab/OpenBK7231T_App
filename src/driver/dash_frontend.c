...
int http_fn_custom_dash(http_request_t *request) {
    ...
    poststr(request,
        ...
        ".trow{display:-webkit-box;-webkit-box-align:center;display:-webkit-flex;display:flex;-webkit-align-items:center;align-items:center;font-size:12px;padding:5px 2px;}"
        ".trow .ic{width:15px;height:15px;margin-right:6px;}"
        ".trow .lbl{color:#7c8a9a;min-width:72px;font-weight:bold;}"
        ".trow .val{color:#e7eef6;font-weight:bold;font-family:'Courier New',monospace;padding-left:4px;}"
    ...
    poststr(request,
        ...
        ".btn-gear{-webkit-box-flex:1.35;-webkit-flex:1.35;flex:1.35;background:#3a3d42;}"
        ...
        ".btn-div{-webkit-box-flex:1.35;-webkit-flex:1.35;flex:1.35;background:#555;}"
    ...
    poststr(request,
        ...
        "</div>"
        "<div class='btn-right'>"
        "<button id='inv-btn' class='btn btn-third' onclick='t_inv()'><span class='btxt'>INVERTER</span></button>"
        ...
        "<input type='range' id='sld-div' min='30' max='255' value='60' oninput='sIn(\"div\",this.value)' onchange='s_div(this.value)'></div>"
        "</div>"
        "</div>"   // btn-box
        "</div>"   // right-col
        "</div>"   // dash-row
    ...
    poststr(request,
        ...
        "if(!held('div')){setV('lbl-div',dthr);setV('sld-div',dthr);}}"
        ...
        "function sIn(k,v){mark(k,parseInt(v,10));if(k==='pwr')setV('lbl-pwr',v);else if(k==='exp')setV('lbl-exp',v);else setV('lbl-div',v);}"
        ...
        "var dmin=texp+10,de=document.getElementById('sld-div');if(de){de.min=dmin;if(parseInt(de.value,10)<dmin){de.value=dmin;dthr=dmin;mark('div',dmin);setV('lbl-div',dmin);cmd('/cm?cmnd=SetDivertThreshold%20'+dmin,null,0);}}}"
        "function s_div(v){var dmin=texp+10;if(parseInt(v,10)<dmin)v=dmin;dthr=parseInt(v,10);mark('div',dthr);cmd('/cm?cmnd=SetDivertThreshold%20'+v,null,0);setV('sld-div',v);setV('lbl-div',v);}"
    );
    rtos_delay_milliseconds(1);
    ...
    poststr(request,
        ...
        "function fixBtns(){var bs=document.querySelectorAll('.btn'),i,b,bh,ch,j,nd;"
        "for(i=0;i<bs.length;i++){b=bs[i];b.style.paddingTop='0';b.style.paddingBottom='0';}"
        "for(i=0;i<bs.length;i++){b=bs[i];bh=b.clientHeight;ch=0;"
        "for(j=0;j<b.childNodes.length;j++){nd=b.childNodes[j];if(nd.nodeType===1)ch+=nd.offsetHeight||0;}"
        "if(bh>ch+2)b.style.paddingTop=Math.round((bh-ch)/2)+'px';}}"
        "initGrid();loadAll();setInterval(runCycle,10000);fit();fixBtns();window.onresize=function(){fit();fixBtns();};setTimeout(function(){fit();fixBtns();},350);"
        "</script></body></html>"
    );
    rtos_delay_milliseconds(1);
