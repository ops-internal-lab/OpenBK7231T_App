...
int http_fn_custom_dash(http_request_t *request) {
    ...
    poststr(request,
        ...
        "if(pe)pe.innerHTML='<span class=\"'+(cpwr<0?'c-exp':'c-imp')+'\">'+cpwr+' W</span>';"
        "setV('d-bal',bal+' Wh');setS('d-bal',bal<0?'#4caf50':'#f44336');"
        "setV('d-est',est+' Wh');setS('d-est',est<0?'#4caf50':'#f44336');"
        "var cv,cc,cx;if(dmp===0){cv='Idle';cc='#888';cx='';}else if(dmp===5){cv='Inverter';cc='#ff9800';cx='';}else if(dmp>=18){cv='Charging';cc='#4caf50';cx=dmp+'%';}else{cv=dmp+'%';cc='#0099FF';cx='';}"
